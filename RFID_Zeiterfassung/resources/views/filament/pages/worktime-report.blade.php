                ...
                @foreach($week['rows'] as $row)
                    @php
                        ...
                        if (! $row['in_month']) {
                            ...
                            $note = \App\Services\WorktimeReport::saldo($row['saldo_roh']);
                        }
                    @endphp
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        @foreach($week['rows'] as $row)
                            <tr @class([
                                ...
                                <td class="py-1 px-2 text-right"
                                    ...
                                    {{ ($row['ist'] || $row['soll']) ? R::saldo($row['saldo']) : '' }}@if($row['toleriert'])<span
                                        style="opacity:.55;font-size:11px;" title="Abweichung {{ R::saldo($row['saldo_roh']) }} liegt unter der Toleranz">°</span>@endif
                                </td>
                        ...
                        <tr class="font-semibold border-t-2">
                            ...
                            <td class="py-1 px-2 text-right">{{ R::saldo($week['sum']['saldo']) }}</td>
                            <td></td>
                        </tr>
                    </tbody>
                </table>
